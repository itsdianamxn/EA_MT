    ...
    with open(jsonl_path, encoding="utf-8") as f:
        for line in f:
            ...
            for qid in rec.get("entities", []):
                ...
                if labels:
                    if i% 2 == 0:
                        ent_rows.append({"input_text": f"{labels[0]}.",  # English
                        ...
                        ent_rows.append({"input_text": labels[0],  # English
                                         "target_text": f'{labels[1]}, "entities":["{labels[1]}"]',  # Translated
                                         "prefix": prefix})
                    ents.append('"' + labels[1] + '"')
            sent_rows.append({"input_text": src,
                              "target_text": f'{tgt}, "entities":[{", ".join(ents)}]',
                              "prefix": prefix})
            i += 1
...
model_args = {
    ...
    "wandb_kwargs": {"name": f"{crt_model}_base_ar_enti_out{datetime.datetime.now().strftime('%m.%d_%H')}"}
}
if __name__ == "__main__":
    ...
    tokenizer = AutoTokenizer.from_pretrained("google/mt5-base", use_fast=False, legacy=False)
