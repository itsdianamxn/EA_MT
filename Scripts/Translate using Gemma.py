...
def translate(lang_id, lang_name):
    line_no = 0;
    with open(f"{PROJECT_ROOT}/Test/{lang_id}_AE.jsonl", 'r', encoding='utf-8') as in_file, \
         open(f"{PROJECT_ROOT}/Results/{lang_id}_trans_gemma.jsonl", 'a', encoding='utf-8') as out_file:
        for line in in_file:
            line_no += 1
            if line_no < 4226: # skip these lines
                continue
            try:
                ...
                prompt = (f"You are an expert translator. Translate the following sentence from English to {lang_name}. " +
                          ...
                          f"\nExample: for the sentence: 'In what genre does A Promised Land fall?' the output will be only: 'في أي نوع تقع الأرض الموعودة ؟'.")
